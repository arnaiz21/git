...
public class Calculadora_Triangulo extends JFrame {
	...
	double resultado, base, altura, lado1, lado2, lado3; // Declaramos variables para las operaciones
	
	char operacion; // Declaramos variable para almacenar el tipo de oeración
	
	String txtbase, txtaltura, txtlado1, txtlado2, txtlado3; // Declaramos variable para almacenar los datos en texto a mostrar
	...
	/**public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Calculadora_Triangulo frame = new Calculadora_Triangulo();
	...
	public Calculadora_Triangulo() {
	  ...
	  btnRetroceso.setFont(new Font("Dialog", Font.BOLD, 16));
		...
		btnArea = new JButton("\u00C1rea");  // Botón de calculo de área del triángulo sobre dato introducido
		...
		btnArea.addActionListener(new ActionListener() {  // ActioListener del botón de cálculo de área
		...
		btnPerimetro = new JButton("Per\u00EDmetro");  // Botón de calculo de perímetro del triángulo sobre dato introducido
		...
		btnPerimetro.addActionListener(new ActionListener() {   // ActioListener del botón de cálculo de perímetro
	  ...
	  btnMasmenos.setEnabled(false);
		...
		btnOK = new JButton("OK");  // Botón para calcular la operación
		...
		btnOK.addActionListener(new ActionListener() {  // ActionListener para el botón de OK
			
			@Override
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				OK();  // Llamada al método OK
			}
		});
		...
		contentPane.add(panelTextos, BorderLayout.SOUTH);
		...
		txtMensaje.setText(" \u25B2 Introduce los valores necesarios, elige la funcion y pulsa OK");
		txtMensaje.setColumns(10);
		txtMensaje.setBorder(dark_grayline);
		
		panelOperaciones = new JPanel(); // JPanel para los datos de texto a introducir
		panelTextos.add(panelOperaciones, BorderLayout.NORTH);
		panelOperaciones.setBackground(Color.DARK_GRAY);
		panelOperaciones.setLayout(new GridLayout(0, 5, 0, 0));
		
		txtBase = new JTextField();  // Campo de texto para recoger la base del triángulo
		panelOperaciones.add(txtBase);
		txtBase.setText("Base"); 
		txtBase.setHorizontalAlignment(SwingConstants.CENTER);
		txtBase.setFont(new Font("Dialog", Font.PLAIN, 14));
		txtBase.setForeground(new Color(105, 105, 105));
		txtBase.setBackground(new Color(192, 192, 192));
		txtBase.setColumns(10);
		txtBase.setBorder(dark_grayline);
		
		txtAltura = new JTextField();  // Campo de texto para recoger la altura del triángulo
		txtAltura.setDropMode(DropMode.INSERT);
		txtAltura.setHorizontalAlignment(SwingConstants.CENTER);
		txtAltura.setFont(new Font("Dialog", Font.PLAIN, 14));
		txtAltura.setForeground(new Color(105, 105, 105));
		txtAltura.setBackground(new Color(192, 192, 192));
		txtAltura.setText("Altura");
		panelOperaciones.add(txtAltura);
		txtAltura.setColumns(10);
		txtAltura.setBorder(dark_grayline);
		
		txtLado1 = new JTextField(); // Campo de texto para recoger el primer lado del triángulo
		txtLado1.setHorizontalAlignment(SwingConstants.CENTER);
		txtLado1.setFont(new Font("Dialog", Font.PLAIN, 14));
		txtLado1.setForeground(new Color(105, 105, 105));
		txtLado1.setBackground(new Color(192, 192, 192));
		txtLado1.setText("Lado 1");
		panelOperaciones.add(txtLado1);
		txtLado1.setColumns(10);
		txtLado1.setBorder(dark_grayline);
		
		txtLado2 = new JTextField();  // Campo de texto para recoger el segundo lado del triángulo
		txtLado2.setHorizontalAlignment(SwingConstants.CENTER);
		txtLado2.setFont(new Font("Dialog", Font.PLAIN, 14));
		txtLado2.setForeground(new Color(105, 105, 105));
		txtLado2.setBackground(new Color(192, 192, 192));
		txtLado2.setText("Lado 2");
		panelOperaciones.add(txtLado2);
		txtLado2.setColumns(10);
		txtLado2.setBorder(dark_grayline);
		
		txtLado3 = new JTextField();  // Campo de texto para recoger el tercer lado del triángulo
		txtLado3.setHorizontalAlignment(SwingConstants.CENTER);
		txtLado3.setFont(new Font("Dialog", Font.PLAIN, 14));
		txtLado3.setForeground(new Color(105, 105, 105));
		txtLado3.setBackground(new Color(192, 192, 192));
		txtLado3.setText("Lado 3");
		panelOperaciones.add(txtLado3);
		txtLado3.setColumns(10);
		txtLado3.setBorder(dark_grayline);
	...
	private void AnyadirFuncion(char funcion) { // método común para registrar una función / si se pulsa algún botón de función

		// if (!nuevaOperacion) { // Si no es una nueva operación
			
			// Almacenar en y el valor introducido haciendo casting a double 
			// altura = Double.parseDouble(textField.getText().replace(',', '.')); // reemplazando coma por punto para el código mostrando en pantalla la coma

			operacion = funcion; // Pasamos el dato del botón a la variable operacion
			
			txtbase = txtBase.getText().replace(',', '.');
			txtaltura = txtAltura.getText().replace(',', '.');
			
			
			txtlado1 = txtLado1.getText().replace(',', '.');
			txtlado2 = txtLado2.getText().replace(',', '.');
			txtlado3 = txtLado3.getText().replace(',', '.');
			...
			case 'a': { // ... en caso de área
				...
				base = Double.parseDouble(txtbase);
				altura = Double.parseDouble(txtaltura);
				
				resultado = base * altura / 2; // operacion del radio
			...
			case 'p': { // ... en caso de perímetro
				...
				lado1 = Double.parseDouble(txtlado1);
				lado2 = Double.parseDouble(txtlado2);
				lado3 = Double.parseDouble(txtlado3);
				
				
				resultado = lado1 + lado2 + lado3; // operacion de perimetro
	...
	private void OK() {
		...
		if (operacion == 'a') {
			...
			txtMensaje.setText("El área del triángulo es: "+ resultado); // muestra en el mensaje informativo el resultado actual si no se pulsa =
		}
		
		else {
			
			txtMensaje.setText("El perímetro del triángulo es: "+ resultado); // muestra en el mensaje informativo el resultado actual si no se pulsa =
	...
	private void BorrarTodo() { // Método para borrar todas las operaciones
		...
		// Reseteamos el contenido de las cajas para los datos
		txtBase.setText("Base");
		txtAltura.setText("Altura");
		txtLado1.setText("Lado 1");
		txtLado2.setText("Lado 2");
