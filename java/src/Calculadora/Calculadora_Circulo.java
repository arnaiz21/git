...
public class Calculadora_Circulo extends JFrame {
	...
	double resultado, radio;  // Declaramos variables para las operaciones
	...
	public Calculadora_Circulo() {
	  ...
	  btnRetroceso.setFont(new Font("Dialog", Font.BOLD, 16));
		...
		btnArea = new JButton("\u00C1rea");  // Botón de calculo de área del círculo sobre dato introducido
		...
		btnPerimetro = new JButton("Per\u00EDmetro"); // Botón de calculo de perímetro del círculo sobre dato introducido
		...
		txtMensaje.setText(" \\u25A0 Introduce el radio del círculo, luego la funci\\u00F3n y después pulsa OK");
	...
	private void AnyadirFuncion(char funcion) { // método común para registrar una función / si se pulsa algun botçon de función
		...
		// if (!nuevaOperacion) { se desactiva la opción de nueva función, serán siempre nuevas
			...
			case 'a': { // ... en caso de área
				...
				resultado = Math.PI * radio * radio; // operacion del área
	...
	private void BorrarTodo() { // Método para borrar todas las operaciones
		...
		resultado = 0;
		textField.setText("0");
		txtMensaje.setText(" \u25A0 Introduce el radio del círculo, luego la funci\u00F3n y después pulsa OK");
